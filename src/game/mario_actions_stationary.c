...
s32 act_first_person(struct MarioState *m) {
    ...
    if (m->floor->type == SURFACE_LOOK_UP_WARP) {
        ...
        u32 unlocked = TRUE;
#else
        u32 unlocked = save_file_get_total_star_count(gCurrSaveFileNum - 1, COURSE_MIN - 1, COURSE_MAX - 1) >= gRequiredStars[STAR_REQ_TOTWC];
#endif

        if (get_intended_level_is_locked(LEVEL_TOTWC)) {
            unlocked = FALSE;
        }
